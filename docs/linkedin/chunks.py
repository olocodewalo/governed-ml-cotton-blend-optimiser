...
def body_html() -> str:
    ...
    # drop the caption line inside the upload markers; captions get typed in later
    raw = re.sub(r'<p class="upload">UPLOAD IMAGE HERE: ([^<]+)<br>.*?</p>',
                 lambda m: f"<p>[[IMG {m.group(1).strip()}]]</p>", raw, flags=re.S)
    raw = re.sub(r'<p class="upload">UPLOAD IMAGE HERE: ([^<]+?)\s*\(([^)]*)\)</p>',
                 lambda m: f"<p>[[IMG {m.group(1).strip()}]]</p>", raw)
